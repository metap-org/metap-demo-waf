...
export function verifyDns(zoneId: string) {
  return apiFetch<DnsVerifyResult>(
    `/api/${ENTITIES.zones}/${zoneId}/verify-dns`,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
...
export function testOrigin(zoneId: string) {
  return apiFetch<OriginTestResult>(
    `/api/${ENTITIES.zones}/${zoneId}/test-origin`,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
...
export function runScanJob(jobId: string) {
  return apiFetch<{ data: { dispatched: boolean; detail: string } }>(
    `/api/${ENTITIES.scanJobs}/${jobId}/run`,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
...
export function evaluateAlerts() {
  return apiFetch<{ data: { policiesEvaluated: number; fired: unknown[] } }>(
...
export function createRecord<T = Record<string, unknown>>(
  ...
  return apiFetch<SingleResponse<T>>(`/api/${entity}`, {
    method: "POST",
...
export function deleteRecord(entity: string, id: string, version: number) {
  return apiFetch<unknown>(`/api/${entity}/${id}`, {
    method: "DELETE",
...
export function transitionRecord<T = Record<string, unknown>>(
  ...
  return apiFetch<SingleResponse<T>>(
